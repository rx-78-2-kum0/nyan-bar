...
export class NyanMode {
  ...
  private _draw() {
    ...
    if (this._option.wavy) {
      ...
      this._renderCache = {
        d: this._ctx.getImageData(0, 0, this._cvs.width, this._cvs.height),
        i: this._i,
      };
    }
  ...
  private _straightRender() {
    if (this._renderCache.d) {
      const diff = this._i - this._renderCache.i;
      if (diff > 0) {
        this._ctx.putImageData(this._renderCache.d, this._rainbowW * diff, 0);
        for (let i = 0; i < diff; i++) {
          this._ctx.putImageData(this._rainbowData, i * this._rainbowW, -this._rainDiff);
        }
      } else {
        this._ctx.putImageData(this._renderCache.d, this._rainbowW * diff, 0);
      }
    } else {
      this._ctx.putImageData(this._catData, 0, 0);
    }
  }

  private _wavyRender() {
    this._ctx.putImageData(this._catData, this._i * this._rainbowW, 0);
    let x = this._i;
    if ((this._i & 1) === 0) {
      while (x--) {
